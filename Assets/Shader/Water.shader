...
Shader "Unlit/Water"
{
	...
	SubShader{
		...
		v2f vert(a2v v){
			v2f o;
			o.pos = UnityObjectToClipPos(v.vertex); //模型空间到裁剪空间的变换函数  unity5.3启用    替换UNITY_MATRIX_MVP矩阵乘法，函数实际还是使用mvp矩阵
		...
		fixed4 frag(v2f i) : SV_Target{
			...
			//使用双采样模拟水面交叉波动
			fixed3 bumpNormal1 = UnpackNormal(tex2D(_WaveMap, i.uv.zw + speed)).rgb; //切线空间下对法线采样需要解码
			fixed3 bumpNormal2 = UnpackNormal(tex2D(_WaveMap, i.uv.zw - speed)).rgb;
			fixed3 bumpNormal = normalize(bumpNormal1 + bumpNormal2); //叠加后的法线
			
			float2 offset = bumpNormal.xy * _Distortion * _RefractionTex_TexelSize.xy;  //折射
			...
			bumpNormal = normalize(half3(dot(i.TtoW0.xyz, bumpNormal), dot(i.TtoW1.xyz, bumpNormal), dot(i.TtoW2.xyz, bumpNormal))); //把法线由切线空间转换到世界空间
			fixed4 texColor = tex2D(_MainTex, i.uv.xy + speed);  //底图颜色采样 speed变量使水面动起来
			fixed3 reflDir = reflect(-viewDir, bumpNormal);  //反射方向
			fixed3 reflCol = texCUBE(_Cubemap, reflDir).rgb * texColor.rgb * _Color.rgb; //反射颜色采样

			fixed fresnel = pow(1 - saturate(dot(viewDir, bumpNormal)), 4);  //菲涅尔系数
